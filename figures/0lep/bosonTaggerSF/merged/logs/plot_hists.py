...
#               ['MTagJetsWithoutTaggerSF'                   , 'm(jj)^{tag} [GeV]'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
               # MVA inputs
#               ['MTagJets'                                 , 'm(jj)^{tag} [GeV]'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
#               ['MFatJet'                                  , 'm(J^{sig}) [GeV]'                    , 10       , 'r'       , 50        , 150        , False      , False                     , False, []],
#               ['PtFatJet'                                  , 'p_{T}(J^{sig}) [GeV]'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
...
#               ['MET'                                      , 'E_{T}^{miss} [GeV]'             , 10       , 'r'       , None        , None        , False      , False                     , False, []],
               ...
               #['NumTrkPt500TagJet1'                             , 'N_{trk}(j^{tag}_{lead})'             , None       , 'r'       , None        , None        , False      , False                     , False, []],
               #['NumTrkPt500TagJet2'                             , 'N_{trk}(j^{tag}_{sublead})'          , None       , 'r'       , None        , None        , False      , False                     , False, []],
               #['NumTrkPt500SignalJet1'                             , 'N_{trk}(j^{sig}_{lead})'             , None       , 'r'       , None        , None        , False      , False                     , False, []],
               #['NumTrkPt500SignalJet2'                             , 'N_{trk}(j^{sig}_{sublead})'          , None       , 'r'       , None        , None        , False      , False                     , False, []],
...
#                ['MVHad'                                    , 'm(jj)^{sig} [GeV]'              , None       , 'r'       ,50        , 150        , False      , False                     , False, []],
#               # vars with tagger SF available
               ['MTagJets'                                 , 'm(jj)^{tag}'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
               ['MTagJetsWithoutTaggerSF'                   , 'm(jj)^{tag}'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
               ['MFatJet'                                 , 'm(J^{sig})'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
               ['MFatJetWithoutTaggerSF'                   , 'm(J^{sig})'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
               ['D2FatJet'                                 , 'D_{2}(J^{sig})'                    , None       , 'r'       , None        , None        , False      , False                     , False, []],
               ['D2FatJetWithoutTaggerSF'                   , 'D_{2}(J^{sig})'                    , None       , 'r'       , None        , None        , False      , False                     , False, []],
               ['PtFatJet'                                 , 'p_{T}(J^{sig})'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
               ['PtFatJetWithoutTaggerSF'                   , 'p_{T}(J^{sig})'                    , 10       , 'r'       , None        , None        , False      , False                     , False, []],
               ['NtrkFatJet'                                 , 'N_{track}(J^{sig})'                    , None       , 'r'       , None        , None        , False      , False                     , False, []],
               ['NtrkFatJetWithoutTaggerSF'                   , 'N_{track}(J^{sig})'                    , None       , 'r'       , None        , None        , False      , False                     , False, []],
               ['EtaFatJet'                                 , '#eta(J^{sig})'                    , None       , 'r'       , None        , None        , False      , False                     , False, []],
               ['EtaFatJetWithoutTaggerSF'                   , '#eta(J^{sig})'                    , None       , 'r'       , None        , None        , False      , False                     , False, []],
...
for var in variables:
  ...
  if 'WithoutTaggerSF' in var[0]: continue

  boson_tagger_sf[var[0]] = {}
  ...
  for region in regions:
    ...
    boson_tagger_sf[var[0]][region_name] = {}

    boson_tagger_sf[var[0]][region_name]['variable'] = var
    ...
    for variable in variables:
      ...
      for category in samples:
        ...
        for channel in samples[category]:
          ...
          #if channel == 'dat': var_name = var_name.replace('WithoutTaggerSF', '') #20220417
      ...
      if not variable[8]: #no_data
        ...
        for key in ratio_keys:
          ...
          else: hists_ratio[key].GetYaxis().SetTitle('with SF / without SF')
      ...
      if do_individual_cut_flow_hists:
        ...
        for var_name in boson_tagger_sf:
          ...
          hist_bkg = stack_envelope(stacks['bkg'])
          hist_bkg.SetFillStyle(0)

          hist_sig = hists['sig'].Clone()
          hist_sig.SetFillStyle(0)

          if var_name + 'WithoutTaggerSF' == variable[0]:

             hist_bkg.SetName('without SF')
             hist_bkg.SetLineColor(ROOT.kBlack)

             boson_tagger_sf[var_name][region_name]['noSFbkg'] = hist_bkg
             boson_tagger_sf[var_name][region_name]['noSFbkg'].SetTitle(';;events')
             boson_tagger_sf[var_name][region_name]['noSFbkg'].SetMinimum(1e-4)

             hist_sig.SetName('without SF')
             hist_sig.SetLineColor(ROOT.kBlack)

             boson_tagger_sf[var_name][region_name]['noSFsig'] = hist_sig
             boson_tagger_sf[var_name][region_name]['noSFsig'].SetTitle(';;events')
             boson_tagger_sf[var_name][region_name]['noSFsig'].SetMinimum(1e-4)
          ...
          if var_name == variable[0]:
             ...
             hist_bkg.SetName('with SF')
             hist_bkg.SetLineColor(ROOT.kRed)

             boson_tagger_sf[var_name][region_name]['withSFbkg'] = hist_bkg
             boson_tagger_sf[var_name][region_name]['withSFbkg'].SetTitle(';;events')
             boson_tagger_sf[var_name][region_name]['withSFbkg'].SetMinimum(1e-4)

             hist_sig.SetName('with SF')
             hist_sig.SetLineColor(ROOT.kRed)

             boson_tagger_sf[var_name][region_name]['withSFsig'] = hist_sig
             boson_tagger_sf[var_name][region_name]['withSFsig'].SetTitle(';;events')
             boson_tagger_sf[var_name][region_name]['withSFsig'].SetMinimum(1e-4)
  ...
  if do_individual_cut_flow_hists:
    ...
    for region in regions:
      ...
      for var_name in boson_tagger_sf:
        ...
        if 'WithoutTaggerSF' in var_name: continue
        ...
        sf_bkg_ratio = sf['withSFbkg'].Clone()
        sf_bkg_ratio.Divide(sf['noSFbkg'])

        text_scale_factor = 3
        sf_bkg_ratio.GetXaxis().SetTitleSize(hists['dat'].GetXaxis().GetTitleSize() * text_scale_factor)
        sf_bkg_ratio.GetXaxis().SetLabelSize(hists['dat'].GetXaxis().GetLabelSize() * text_scale_factor)
        sf_bkg_ratio.GetYaxis().SetTitleSize(hists['dat'].GetYaxis().GetTitleSize() * text_scale_factor)
        sf_bkg_ratio.GetYaxis().SetLabelSize(hists['dat'].GetYaxis().GetLabelSize() * text_scale_factor)
        sf_bkg_ratio.GetYaxis().SetNdivisions(3)

        sf_bkg_ratio.SetFillColor(0)
        sf_bkg_ratio.SetMarkerStyle(8)
        sf_bkg_ratio.SetLineColor(ROOT.kBlack)
        sf_bkg_ratio.SetMarkerColor(ROOT.kBlack)

        sf_bkg_ratio.GetXaxis().SetTitle(sf['variable'][1])
        sf_bkg_ratio.GetYaxis().SetTitle('with/without')

        make_plot('sf_bkg', region_name, sf['variable'], None, [sf['noSFbkg'], sf['withSFbkg']], [sf_bkg_ratio], sf['dat'], [], ratio_margin=.2)

        sf_sig_ratio = sf['withSFsig'].Clone()
        sf_sig_ratio.Divide(sf['noSFsig'])

        text_scale_factor = 3
        sf_sig_ratio.GetXaxis().SetTitleSize(hists['dat'].GetXaxis().GetTitleSize() * text_scale_factor)
        sf_sig_ratio.GetXaxis().SetLabelSize(hists['dat'].GetXaxis().GetLabelSize() * text_scale_factor)
        sf_sig_ratio.GetYaxis().SetTitleSize(hists['dat'].GetYaxis().GetTitleSize() * text_scale_factor)
        sf_sig_ratio.GetYaxis().SetLabelSize(hists['dat'].GetYaxis().GetLabelSize() * text_scale_factor)
        sf_sig_ratio.GetYaxis().SetNdivisions(3)

        sf_sig_ratio.SetFillColor(0)
        sf_sig_ratio.SetMarkerStyle(8)
        sf_sig_ratio.SetLineColor(ROOT.kBlack)
        sf_sig_ratio.SetMarkerColor(ROOT.kBlack)

        sf_sig_ratio.GetXaxis().SetTitle(sf['variable'][1])
        sf_sig_ratio.GetYaxis().SetTitle('with/without')

        make_plot('sf_sig', region_name, sf['variable'], None, [sf['noSFsig'], sf['withSFsig']], [sf_sig_ratio], sf['dat'], [], ratio_margin=.2)
...
def make_plot(prefix, region_name, variable, stack_main, contours_main, hists_ratio, hist_dat, hists_soverb, hist_dat_extra = None, do_ratio_fit = False, altovernom_margin = 2., ratio_margin = .45, cutflow_hists = None, th2 = False, wide_legend = False):
  ...
  if stack_main != None or len(contours_main) > 0 or len(hists_ratio) > 0 or hist_dat != None and False:
    ...
    legend_main = ROOT.TLegend(0.2,0.7,0.5,0.9) if variable[3] == 'l' else ROOT.TLegend(0.6,0.7,.9,0.9)
    if wide_legend:
    ...
    if stack_main != None:
      ...
      if len(hists_soverb) > 0 and not th2:
        ...
        stack_main.GetYaxis().SetTitleOffset(stack_main.GetYaxis().GetTitleOffset() * .8)
