...
samples['bkg']['bin0'] = ['EW60LepFid0']
#samples['bkg']['bin1'] = ['EW60LepFid1']
samples['bkg']['bin2'] = ['EW60LepFid2']
samples['bkg']['bin3'] = ['EW60LepFid3']
...
samples['sig']['sig'] = ['EW60LepFid1']
#samples['sig']['sig'] = ['EW60LepFid0', 'EW60LepFid1', 'EW60LepFid2', 'EW60LepFid3'] 
samples['dat']['dat'] = ['EW60LepFid0', 'EW60LepFid1', 'EW60LepFid2', 'EW60LepFid3'] 
...
colours['bkg']['bin0'] = ROOT.kGreen
colours['bkg']['bin1'] = ROOT.kRed
colours['bkg']['bin2'] = ROOT.kBlue
colours['bkg']['bin3'] = ROOT.kViolet
...
def main():
  ...
  if len(sys.argv) > 3:
    cutflow_type = sys.argv[3]
    if cutflow_type == 'all': cutflow_type = ''
    print('cutflow_type:', cutflow_type)
    ...
    for variable in variables:
        ...
        for channel in samples[category]:
          ...
          # only one region
            ...
            if var_name.startswith('cutflow'):
              ...
              if channel_is_alt and alt_samples_available:
                channel_hists = flatten_list([[key for key in cutflow_keys_alt if key.GetName() == '_'.join([sample, ((cutflow_type + 'SeqCutsRaw') if 'raw' in var_name else (cutflow_type + 'SeqCuts')) + region_name])] for sample in samples[category][channel]])
              else:
                channel_hists = flatten_list([[key for key in cutflow_keys if key.GetName() == '_'.join([sample, ((cutflow_type + 'SeqCutsRaw') if 'raw' in var_name else (cutflow_type + 'SeqCuts')) + region_name])] for sample in samples[category][channel]])
            else:
      ...
      if not variable[8]: #no_data
        ...
        for key in ratio_keys:
          ...
          else: hists_ratio[key].GetYaxis().SetTitle('fiducial / all')
          ...
          else: hists_ratio[key].Divide(hists['sig'])
      ...
      if do_individual_cut_flow_hists:
        ...
        if not variable[8]: #no_data
          # nominal plots
          make_plot('nom', region_name, variable, stacks['nom'], [], [hists_ratio['nom']], None, [], cutflow_hists = [hists['dat'], stack_envelope(stacks['nom']), stack_envelope(stacks['bkg'])], cutflow_type=cutflow_type)

          ## nominal but with s/b ratio
          ##make_plot('soverb', region_name, variable, stacks['nom'], [], [], None, [hists_ratio['soverb_nom']], cutflow_hists = [hists['dat'], stack_envelope(stacks['nom']), stack_envelope(stacks['bkg'])])
...
def make_plot(prefix, region_name, variable, stack_main, contours_main, hists_ratio, hist_dat, hists_soverb, hist_dat_extra = None, do_ratio_fit = False, altovernom_margin = 2., ratio_margin = .45, cutflow_hists = None, th2 = False, cutflow_type=None):
  ...
  if stack_main != None or len(contours_main) > 0 or len(hists_ratio) > 0 or hist_dat != None and False:
    ...
    if stack_main != None:
      ...
      if variable[0].startswith('cutflow'): stack_main.SetMinimum(0.)
      ...
      if variable[0].startswith('cutflow'): hist_dat.SetMinimum(0.)
      ...
      for ctr, hist in enumerate(contours_main):
        ...
        if variable[0].startswith('cutflow'): hist.SetMinimum(0.)
...
#    atlas_tex.SetTextAlign(13) # align at top
  ...
  # make cutflow tex table
    ...
    if variable[0].startswith('cutflow') and prefix == 'nom':
      ...
      with open(out_dir + '/' + cutflow_type + prefix + '_' + variable[0] + '_' + region_name + '.tex', 'w') as tex_file:

        sys.stdout = tex_file

        print('\\begin{tabular}{r|c|c|c|c}')
        print('cut&fiducial&non-fiducial&all signal&fiducial/all\\\\')
        print('\hline')
        ...
        for b in range(1, hst.GetNbinsX() + 1):
          ...
          dat_over_mc =  None if (num_sig == None or num_mcall == 0) else num_sig / num_mcall
  ...
  can.Print(out_dir + '/plots/' + cutflow_type + plot_name + '.pdf')

  can.GetListOfPrimitives().Delete()

def manipulate_legend_name(string):
  
  string = string.replace('sig', 'bin1')
